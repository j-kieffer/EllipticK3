...
declare attributes EllK3: EC, // Elliptic curve over univariate function field
        ...
        RootNSLat, // Intersection with subspace generated by roots
...
intrinsic NeronSeveriLattice(S :: EllK3) -> Lat
{Essential part of Néron--Severi lattice of S}
    require assigned S`NSLat: "Néron--Severi lattice not assigned";
    return S`NSLat;
end intrinsic;

intrinsic NeronSeveriRootSpan(S :: EllK3) -> Lat
{Part of Néron--Severi lattice of S contained in the span of roots}
    require assigned S`RootNSLat: "Néron--Severi lattice not assigned";
    return S`RootNSLat;
end intrinsic;
...
intrinsic NeronSeveriAmbientSpace(S :: EllK3) -> ModTupFld
{Ambient space of Néron--Severi lattice of S}
    return AmbientSpace(NeronSeveriLattice(S));
end intrinsic;

intrinsic NeronSeveriRank(S :: EllK3) -> RngIntElt
{Rank of known Néron--Severi lattice of S}
    return Rank(NeronSeveriLattice(S)) + 2;
end intrinsic;
...
{True iff reducible fiber components are defined over the base field}
    require assigned fib`Field: "Field not assigned";
    return fib`Field;
end intrinsic;
